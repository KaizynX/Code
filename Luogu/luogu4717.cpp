 ...
 * @LastEditTime: 2020-08-12 14:31:16
 */
#include <bits/stdc++.h>
...
#define forforfor for (int l = 2; l <= len; l <<= 1)\
  ...
  template <class T> void fwt_or(T a[], const int x = 1) {
    forforfor (a[i+j+k] += a[i+j]*x) %= MOD;
  }
  template <class T> void fwt_and(T a[], const int x = 1) {
    forforfor (a[i+j] += a[i+j+k]*x) %= MOD;
  }
  template <class T> void fwt_xor(T a[], const int x = 1) {
    forforfor {
      (a[i+j] += a[i+j+k]) %= MOD;
      a[i+j+k] = (a[i+j]-a[i+j+k]-a[i+j+k]+2*MOD)%MOD;
      (a[i+j] *= x) %= MOD; (a[i+j+k] *= x) %= MOD;
    }
  }
