/*
 * @Author: Kaizyn
 * @Date: 2021-08-25 16:31:28
 * @LastEditTime: 2021-08-25 16:40:46
 */
#include <bits/stdc++.h>
...
template <typename T>
void update(int i, int j, int k, T y) {
  if (y > dp[i][j]) {
    dp[i][j] = y;
    path[i][j] = k;
  }
...
struct generalSAM {
    ...
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < sz; ++j) if (dp[i][j] > -100.0) {
        if (T[i] == '.') for (int k = 0; k < A; ++k) {
          ...
          update(i+1, nex[j][k], j, dp[i][j]+val[nex[j][k]]);
        } else {
          // update(dp[i+1][nex[j][T[i]-C]], dp[i][j]+val[nex[j][T[i]-C]]);
          update(i+1, nex[j][T[i]-C], j, dp[i][j]+val[nex[j][T[i]-C]]);
        }
      }
    }
    #ifdef DEBUG
  ...
  string print() {
    ...
    string ans;
    for (int i = n-1, v; i >= 0; --i, u = v) {
      v = path[i+1][u];
      if (T[i] == '.') for (int k = 0; k < A; ++k) {
        if (nex[v][k] != u) continue;
        if (cmp(dp[i+1][u], dp[i][v]+val[u])) {
          ans += C+k;
          break;
        }
      } else if (nex[v][T[i]-C]) {
