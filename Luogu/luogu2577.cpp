...
int main()
{
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 0; j <= i; ++j) {
            for (int k = 0; k <= s[i]; ++k) {
                if (dp[i&1^1][j][k] != -1)
                    update(dp[i&1][j][k], max(dp[i&1^1][j][k], s[i]-k+a[i].t2));
                if (j > 0 && k-a[i].t1 >= 0 && dp[i&1^1][j-1][k-a[i].t1] != -1)
                    update(dp[i&1][j][k], max(dp[i&1^1][j-1][k-a[i].t1], k+a[i].t2));
#ifdef DEBUG
                printf("[%d][%d][%d] %d\n", i, j, k, dp[i&1][j][k]);
#endif
            }
