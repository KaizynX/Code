/*
 * @Author: Kaizyn
 * @Date: 2021-07-09 22:29:04
 * @LastEditTime: 2021-07-09 22:45:22
 */
#include <bits/stdc++.h>
...
template <typename T>
inline Mint get_val(const int &u, const T &x) {
  int p = x%P;
  for (int i = head[u][p]; i; i = nex[u][i]) {
  ...
  for (int i = 1; i <= n; ++i) {
    for (int j = 0; j < m; ++j) {
      ...
      for (int k = 1; k <= tot[u^1]; ++k) {
        ...
        int b1 = j ? (state>>(j*2-2))%4 : 2, b2 = (state>>(j*2))%4;
        if (j) {
          if (b2 == 2) {
            insert(u, state-(1<<(j*2)), val); // 上0下1
            insert(u, state-(2<<(j*2)), val); // 上1下0
          } else {
            if (b1 == 2) {
              insert(u, state-(b2<<(j*2))-(1<<(j*2-2)), val); // 左1右0
              insert(u, (state^((b2^1)<<(j*2)))-(2<<(j*2-2)), val); // 左0右1
            }
            insert(u, state-(b2<<(j*2)), val);
          }
        } else {
          if (b2 == 2) {
            insert(u, state-(1<<(j*2)), val);
