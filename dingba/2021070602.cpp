/*
 * @Author: Kaizyn
 * @Date: 2021-07-07 19:50:47
 * @LastEditTime: 2021-07-07 20:52:46
 */
#include <bits/stdc++.h>
...
int fa[N<<1], dfn[N], from[N], tp[N], bm[N];
vector<int> e[N], ce[N<<1];
...
bool dfs(int u) {
  ...
  for (const int &v : e[u]) {
    ...
    if (!dfn[v]) {
      ...
      if (!from[v]) ce[u].emplace_back(v);
    } else if (dfn[v] < dfn[u]) {
      ++cnt;
      tp[cnt] = v; bm[cnt] = u;
      ce[v].emplace_back(n+cnt);
      for (int x = u; x != v; x = fa[x]) {
        if (from[x]) return false;
        from[x] = cnt;
        ce[n+cnt].emplace_back(x);
      }
    }
  }
  return true;
