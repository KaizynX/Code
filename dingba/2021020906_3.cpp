 ...
 * @LastEditTime: 2021-02-09 22:37:47
 */
#include <bits/stdc++.h>
...
signed main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    NTT::work(pre[i], n+1, g[i], n+1);
  }
  for (int i = n; i; --i) {
    memcpy(suf[i], suf[i+1], sizeof(suf[i]));
    NTT::work(suf[i], n+1, g[i], n+1);
  }
  for (int i = 1; i <= n; ++i) {
    memset(f, 0, sizeof(int)*(n+1));
    f[i] = comb.inv[i];
    NTT::work(f, n+1, pre[i-1], n+1);
    NTT::work(f, n+1, suf[i+1], n+1);
    (res += f[n]) %= MOD;
  }
