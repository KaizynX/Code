/*
 * @Author: Kaizyn
 * @Date: 2021-02-25 20:34:30
 * @LastEditTime: 2021-02-25 20:39:28
 */
#include <bits/stdc++.h>
...
int fuck(int x, int y) {
  if (y == 1) swap(x, y);
  int res = __builtin_popcount(shit[x]^shit[y]);
  if (x == 1) res = min(res, __builtin_popcount(shit[10]^shit[y]));
  return res;
}
