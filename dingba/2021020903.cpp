/*
 * @Author: Kaizyn
 * @Date: 2021-02-09 20:18:14
 * @LastEditTime: 2021-02-09 20:45:19
 */
#include <bits/stdc++.h>
...
int a[N], l[N], r[N];
Mint dp[N];
PersistantSegmentTree<Mint> tree;
...
inline void solve() {
  ...
  Mint res = dp[1];
  tree.update(1, 0, a[1], dp[1]);
  for (int i = 2; i <= n; ++i) {
    dp[i] = tree.query(l[i], r[i], 1, a[i])/(r[i]-l[i]+1);
    res += dp[i];
    tree.update(i, i-1, a[i], dp[i]);
  }
  #ifdef DEBUG
  for (int i = 1; i <= n; ++i) {
    cout << dp[i] << " \n"[i==n];
