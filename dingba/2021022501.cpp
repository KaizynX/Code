/*
 * @Author: Kaizyn
 * @Date: 2021-02-25 18:29:51
 * @LastEditTime: 2021-02-25 19:54:21
 */
#include <bits/stdc++.h>
...
void init() {
  ...
  for (int i = 10; i < 1000; ++i) {
    for (int j = 10; j < i; ++j) if (cnt(i) == cnt(j)) {
      magic[i][j] = magic[j][i] = init2(i, j);
    }
  }
  for (int i = 0; i < 1000; ++i) {
    for (int _ = cnt(i), t = 1; _; --_, t *= 10) {
      cnt2[i] += __builtin_popcount(i/t%10);
    }
  }
}
