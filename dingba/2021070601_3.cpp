/*
 * @Author: Kaizyn
 * @Date: 2021-07-07 15:16:12
 * @LastEditTime: 2021-07-07 16:37:17
 */
#include <bits/stdc++.h>
  ...
  for (const pil &edge : e[u]) {
    ...
    if (!dfn[v]) {
      ...
      for (int x = u; x != v; x = f[x][0]) {
        ...
        from[x] = cnt;
        cir[cnt] += od[x]-od[f[x][0]];
      }
      for (int x = u; x != v; x = f[x][0]) {
        ll ww = od[x]-od[v];
        add_edge(x, n+cnt, min(ww, cir[cnt]-ww));
      }
    }
  }
  return true;
...
ll query(int u, int v) {
  ...
  ll d = abs(od[x]-od[y]);
  return dis[u]-dis[x]+dis[v]-dis[y]+min(d, cir[f[x][0]]-d);
}
