 ...
 * @LastEditTime: 2020-05-26 20:10:11
 */ 
#include <bits/stdc++.h>
...
inline void solve() {
  ...
  for (int i = 1; i <= n; ++i) { // length
    for (int j = 0; j < m; ++j) if (dp[i-1][j] >= 0) { // pos at ac
      for (int k = 0; k < 26; ++k) {
        // (i-1)'th at j --> i'th chhose char(k) at np
        int np = ac.nex[j][k],
            val = dp[i-1][j]+ac.num[np];
        if (val > dp[i][np] || (val == dp[i][np] && ac.ch[i] < ac.ch[pre[i][np]])) {
          dp[i][np] = val;
          pre[i][np] = j;
