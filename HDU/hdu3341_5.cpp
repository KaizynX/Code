 ...
 * @LastEditTime: 2020-05-28 19:21:57
 */ 
#include <bits/stdc++.h>
...
int n;
int num[4], state[4], w[507], base[4];
char s[M];
int dp[507][H];
...
inline void solve(const int &T) {
  ...
  base[0] = 1;
  base[1] = num[0]+1;
  base[2] = base[1]*(num[1]+1);
  base[3] = base[2]*(num[2]+1);
  n = ac.cnt+1;
  int res = 0;
  memset(state, 0, sizeof state);
  for (int i = 0, sz = base[3]*(num[3]+1); i < n; ++i) {
  ...
  for (state[3] = 0; state[3] <= num[3]; ++state[3]) {
    hs = 0;
    for (int i = 0; i < 4; ++i) hs += state[i]*base[i];
    for (int i = 0; i < n; ++i) if (dp[i][hs] >= 0) {
      for (int j = 0; j < 4; ++j) if (state[j]+1 <= num[j]) {
        int np = ac.nex[i][j], val = dp[i][hs]+w[np];
        dp[np][hs+base[j]] = max(dp[np][hs+base[j]], val);
        res = max(res, val);
      }
    }
  }
