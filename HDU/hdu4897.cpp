 ...
 * @LastEditTime: 2020-06-09 22:59:34
 */ 
#include <bits/stdc++.h>
...
struct ShuPou {
  ...
  T query(int x, int y) {
    ...
    while (tp[x] != tp[y]) {
      if (d[tp[x]] < d[tp[y]]) swap(x, y);
      if (tp[x] == x) {
        // res += hv.query(id[x])^lt.query(id[x])^lt.query(id[fa[x]]);
        res += hv.query(id[x])^lt.query(id[fa[x]]);
        x = fa[x];
      } else {
        res += hv.query(id[tp[x]]+1, id[x]);
        x = tp[x];
      }
    }
    // must be heavy chain
    if (d[x] > d[y]) swap(x, y);
    if (x != y) res += hv.query(id[x]+1, id[y]);
    return res;
  }
};
