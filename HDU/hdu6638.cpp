...
inline void push_up(int i)
{
    ...
    tr[i].ls = max(tr[i].ls, max(tr[i<<1].ls, tr[i<<1].s+tr[i<<1|1].ls));
    tr[i].rs = max(tr[i].rs, max(tr[i<<1|1].rs, tr[i<<1|1].s+tr[i<<1].rs));
    tr[i].ms = max( max(tr[i].ms, tr[i<<1].rs+tr[i<<1|1].ls),
                    max(tr[i<<1].ms, tr[i<<1|1].ms)          );
}
...
{
    tr[i].ls = tr[i].rs = tr[i].ms = max(0ll, (tr[i].s += w));
}

void update_tree(int i, int y, int w)
{
    if (tr[i].l == y && tr[i].r == y) {
        update_leaf(i, w);
        return;
    }
    int mid = (tr[i].l+tr[i].r)/2;
    if(y <= mid) update_tree(i<<1, y, w);
    else update_tree(i<<1|1, y, w);
    push_up(i);
}
