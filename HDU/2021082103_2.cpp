/*
 * @Author: Kaizyn
 * @Date: 2021-08-21 13:49:01
 * @LastEditTime: 2021-08-21 13:55:58
 */
/*
 * @Author: Kaizyn
...
inline void solve() {
  ...
  int S = sum+1, T = sum+2, SS = sum+3;
  flow.init(sum+3);
  flow.add_edge(SS, S, m, 0);
  flow.add_edge(S, T, m, 0); // useless nodes
  for (int i = 1, j, k, id = 1; i <= n; ++i) {
    flow.add_edge(S, id, t[i], 0);
    for (j = 2, k = 1; j <= t[i]; ++j) {
      ++id;
      flow.add_edge(id-1, id, t[i], 0);
      int vj = (a[i]+j-1)/j, vk = (a[i]+k-1)/k;
      if (vj != vk) {
        flow.add_edge(id-1, id, 0, vj-vk);
        k = j;
      }
    }
  }
