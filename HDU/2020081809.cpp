 ...
 * @LastEditTime: 2020-08-18 13:17:14
 */
#include <bits/stdc++.h>
    ...
    for (int j = 1; j <= n; ++j) {
      ...
      for (int k = 1; k <= tot[u^1]; ++k) {
        ...
        if (!b1 && !b2) {
          ...
          insert(u, state+(3<<j*2-2)+(3<<j*2), val+a[i][j]);
        } else if (!b1 && b2 == 3) {
          if (i < n && j < n) insert(u, state+(1<<j*2-2)-(1<<j*2), val);
        } else if (b1 == 3 && !b2) {
          if (i < n && j < n) insert(u, state-(2<<j*2-2)+(2<<j*2), val);
        } else if (b1 == 3 && b2 == 3) {
          if (i < n && j < n) insert(u, state-(2<<j*2-2)-(1<<j*2), val);
        } else if ((b1 == 0 || b1 == 3) && b2) {
          if (b1 == 3) state -= 3<<j*2-2;
          if (j < n) insert(u, state, val);
          if (i < n) insert(u, state+(b2<<j*2-2)-(b2<<j*2), val);
        } else if (b1 && (b2 == 0 || b2 == 3)) {
          if (b2 == 3) state -= 3<<j*2;
          ...
          for (int k = j-2, t = 1; k >= 0; --k) {
            if ((state>>k*2)%4 == 1) --t;
            if ((state>>k*2)%4 == 2) ++t;
            if (!t) { insert(u, state-(1<<j*2-1)-(1<<j*2+1)+(1<<k*2), val); break; }
          }
        } else if (b1 == 2 && b2 == 1) {
          ...
          if (i < n) continue;
          if (j == n-1) {
            if ((state>>n*2) != 3) res = max(res, val+a[n][n]);
          } else if (j == n) {
            res = max(res, val);
          }
        }
