 ...
 * @LastEditTime: 2020-08-06 16:54:00
 */
#include <bits/stdc++.h>
...
inline void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < m; ++j) {
      ...
      while (t != j && (a[rk[t]]-a[i])*(a[rk[j]]-a[i]) <= 0) {
        mul *= -a[rk[t]].p+1;
        t = (t+1)%m;
      }
      */
      ...
      int k;
      for (k = 1; k < m; ++k) {
        // if (i == 2 && rk[j] == 3 && rk[(j+k)%m] == 1) cout << (a[rk[(j+k)%m]]-a[i])*(a[rk[j]]-a[i]) << endl;
        if ((a[rk[(j-k+m)%m]]-a[i])*(a[rk[j]]-a[i]) <= 0) {
          mul *= -a[rk[(j-k+m)%m]].p+1;
        } else {
          break;
        }
      }
