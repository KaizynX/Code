 ...
 * @LastEditTime: 2020-08-06 21:26:56
 */
#include <bits/stdc++.h>
...
inline void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < m; ++j) {
      int l = j+1, r = j+m;
      while (b[j]*b[l%m] > 0) ++l;
      while ((r+1)%m != l%m && b[j]*b[(r+1)%m] <= 0) ++r;
      cout << j << " " << l << " " << r << endl;
      mint tmp = a[i].p*b[j].p*(a[i]*(b[j]+a[i]));
      for (int k = l; k <= r; ++k) tmp *= -b[k%m].p+1;
      res += tmp/(-b[j].p+1);
      tmp = a[i].p*b[j].p*(a[i]*(b[j]+a[i]));
      for (int k = 0; k < m; ++k) if (j != k && b[j]*b[k] <= 0) tmp *= -b[k].p+1;
      res2 += tmp;
    }
  }
  cout << res/2 << " " << -res/2 << endl;
  cout << res2/2 << " " << -res2/2 << endl;
}
