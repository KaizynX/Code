/*
 * @Author: Kaizyn
 * @Date: 2021-08-21 15:26:50
 * @LastEditTime: 2021-08-21 16:13:55
 */
#ifdef ONLINE_JUDGE
...
struct generalSAM {
  ...
  void init() {
    ...
    for (int i = 0; i < sz; ++i) qry[i].clear();
    link[0] = -1; sz = 1;
  }
  ...
  void solve() {
    for (int _ = sz-1, i, j; _; --_) { // assert(rk[0] == 0);
      ...
      int d = tree.query(tr[i]);
      for (auto k : qry[i]) ans[k.second] = d < k.first;
      tree.merge(1, n, tr[j], tr[i]);
    }
  }
  void query(int pl, int pr, int id) {
    int u = ed[pr];
    for (int j = d[u]; j >= 0; --j) {
      if (len[fa[u][j]] >= pr-pl+1) u = fa[u][j];
    }
    qry[u].emplace_back(pr-pl+1, id);
