/*
 * @Author: Kaizyn
 * @Date: 2020-08-25 10:15:41
 * @LastEditTime: 2020-08-25 10:18:42
 */
...
#define magic(x) (1+x-l)
  ...
  m = 1+r-l;
  tr.init(m);
  for (int i = 1; i <= m; ++i) q[i].insert(-INF);
  dp[0] = 0;
  // add sum=0, dp=0
  for (int i = 1; i <= n; ++i) {
    if (i-L >= 0) my_add(magic(sum[i-L]), dp[i-L]);
    if (i-R-1 >= 0) my_del(magic(sum[i-R-1]), dp[i-R-1]);
