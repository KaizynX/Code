 ...
 * @LastEditTime: 2020-06-04 16:18:30
 */ 
#include <bits/stdc++.h>
...
inline void discrete() {
  static int m;
  dis[0] = -1;
  memcpy(dis+1, a+1, sizeof(int)*n);
  sort(dis+1, dis+n+1);
  m = unique(dis+1, dis+n+1)-dis;
  for (int i = 1; i <= n; ++i) {
    a[i] = lower_bound(dis+1, dis+m, a[i])-dis;
  }
}
