 ...
 * @LastEditTime: 2020-07-21 13:17:39
 */ 
#include <bits/stdc++.h>
...
tree<pii,null_type,less<pii>,rb_tree_tag,tree_order_statistics_node_update> t[N]; //红黑树

inline int query(const int &u) {
  int l = 0, r = 1e9, mid;
  while (l < r) {
    mid = (l+r)>>1;
    if ((int)t[u].order_of_key({mid+1, 0}) == mid+1) l = mid+1;
    else r = mid;
  }
  return l;
}
...
inline void solve() {
  ...
  for (int i = 1; i <= m; ++i) {
    scanf("%d %d", eu+i, ev+i);
    ++deg[eu[i]]; ++deg[ev[i]];
    t[eu[i]].insert({a[ev[i]], ev[i]});
    t[ev[i]].insert({a[eu[i]], eu[i]});
  }
  sqr = sqrt(2*m);
  ...
  for (int i = 1, op, u, x; i <= q; ++i) {
    ...
    if (op == 1) {
      ...
      if (!tag[u]) { // light point rebuild
        t[u].clear();
        for (const int &v : hve[u]) t[u].insert({a[v], v});
        for (const int &v : lte[u]) t[u].insert({a[v], v});
      }
      printf("%d\n", query(u));
    }
  }
