...
struct FenKuai
{
    ...
    {
        int i = x;
        modify(x/t); for ( ; i <= y && i%t; ++i) a[i] += k;
        for ( ; i+t-1 <= y; i += t) tag_add[i/t] += k;
        modify(y/t); for ( ; i <= y; ++i) a[i] += k;
    }
    ...
    void mul(int x, int y, int k)
    {
        int i = x;
        modify(x/t); for ( ; i <= y && i%t; ++i) a[i] *= k;
        for ( ; i+t-1 <= y; i += t) tag_mul[i/t] *= k, tag_add[i/t] *= k;
        modify(y/t); for ( ; i <= y; ++i) a[i] *= k;
    }
...
{
    scanf("%d", &n);
    for (int i = 0; i < n; ++i) scanf("%d", &B.a[i].v);
    B.init(n);
