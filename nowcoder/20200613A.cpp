/*
 * @Author: Kaizyn
 * @Date: 2020-06-13 12:47:31
 * @LastEditTime: 2020-06-13 13:53:34
 */ 
#include <bits/stdc++.h>
...
inline void solve() {
  ...
  num[n+1] = 0;
  for (int i = n; i; --i) {
    num[i] = num[i+1]+pw10[n-i]*(s[i-1]-'0');
  }
  for (int i = 1; i <= n; ++i) {
    res += sum[n-i]*(s[i-1]-'0');
    if (s[i-1] == '7') res += num[i+1]+1;
    else if (s[i-1] > '7') res += pw10[n-i];
  }
  cout << res << endl;
}
...
signed main() {
  ...
  for (int i = 1; i <= 100000; ++i) {
    pw10[i] = pw10[i-1]*10;
    cnt[i] = cnt[i-1]*9+pw10[i-1];
    sum[i] = sum[i-1]*10+pw10[i-1];
  }
  int T = 1;
  cin >> T;
  for (int t = 1; t <= T; ++t) {
