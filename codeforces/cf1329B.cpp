/*
 * @Author: Kaizyn
 * @Date: 2020-04-03 23:03:34
 * @LastEditTime: 2020-04-03 23:38:09
 */
#include <bits/stdc++.h>
...
{
    cin >> d >> MOD;
    if (d == 1) return void(cout << 1%MOD << endl);
    if (d == 2) return void(cout << 3%MOD << endl);
    c2[0] = c1[0] = 1;
    for (int i = 1; i <= 30; ++i) {
        c2[i] = c1[i-1]*2+1;
        c1[i] = c2[i]+(c1[i-1]+1)*((1<<i)-1);
    }
    int b = 0;
    while (1<<(b+1) <= d) ++b;
    cout << c2[b]+(c1[b-1]+1)*(d-(1<<b)) << endl;
}
