/*
 * @Author: Kaizyn
 * @Date: 2020-08-15 16:53:44
 * @LastEditTime: 2020-08-16 13:47:26
 */
#include <bits/stdc++.h>
...
int n;
int s[1<<B], a[1<<B], b[1<<B], c[1<<B], fib[1<<B], aor[1<<B], axor[1<<B];
...
signed main() {
  ...
  // aor[i] the number of pair sa|ab == i
  FWT::work_or(s, 1<<B, s, 1<<B);
  memcpy(aor, FWT::f, sizeof aor);
  // for (int i = 0; i < 1<<B; ++i) aor[i] = FWT::f[i]*2%MOD;
  // axor[i] the number of pair sa^ab == i
  FWT::work_xor(s, 1<<B, s, 1<<B);
  memcpy(axor, FWT::f, sizeof axor);
#ifdef DEBUG
  printf("fib:"); print(fib);
  printf("or:"); print(aor);
  printf("xor:"); print(axor);
#endif
  // for (int i = 0; i < 1<<B; ++i) axor[i] = FWT::f[i]*2%MOD;
  int res = 0;
  for (int bit = 0; bit < B; ++bit) {
    ...
    for (int i = 0; i < 1<<B; ++i) if ((i>>bit)&1) {
      a[i^(1<<bit)] = 1ll*aor[i]*fib[i]%MOD;
      b[i^(1<<bit)] = 1ll*s[i]*fib[i]%MOD;
      c[i^(1<<bit)] = 1ll*axor[i]*fib[i]%MOD;
    }
#ifdef DEBUG
    printf("2^i=%d\n", 1<<bit);
    printf("a:"); print(a);
    ...
    memcpy(a, FST::res, sizeof a);
    FST::work(a, 1<<B, c, 1<<B);
#ifdef DEBUG
