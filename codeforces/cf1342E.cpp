/*
 * @Author: Kaizyn
 * @Date: 2020-04-27 00:07:14
 * @LastEditTime: 2020-04-27 09:31:42
 */
#include <bits/stdc++.h>
...
mint calc() {
    return fac[n]*fac[n-1]/(fac[k]*fac[k]*fac[n-k-1]);
}
...
inline void solve()
{
    ...
    if (k == 0) return void(cout << fac[n] << endl);
    // mint res = mint(n-k).pow(n);
    mint res = 0;
    for (int i = 0; i <= n-k; ++i) {
        if (i&1) res -= C(n-k, i)*mint(n-k-i).pow(n);
        else res += C(n-k, i)*mint(n-k-i).pow(n);
    }
    res *= C(n, n-k);
    cout << res*2 << endl;
}
