/*
 * @Author: Kaizyn
 * @Date: 2022-03-09 14:32:42
 * @LastEditTime: 2022-03-09 16:09:21
 */
#include <bits/stdc++.h>
...
struct TireTree {
  ...
  void rebuild() {
    memcpy(idx + 1, help_idx + 1, sizeof(int) * n);
    for (int i = 1; i <= cnt; ++i) {
      vector<int> tmp(has[i].size());
      for (unsigned j = 0; j < has[i].size(); ++j) {
        tmp[j] = changes.count(has[i][j]) ? changes[has[i][j]] : has[i][j];
      }
