/*
 * @Author: Kaizyn
 * @Date: 2021-11-09 11:07:05
 * @LastEditTime: 2021-11-09 11:42:01
 */
#include <bits/stdc++.h>
...
inline void solve() {
  ...
  cnt[n&1].update(a[n], 1);
  for (int i = n-1, j, k, l, r; i; --i) {
    // cnt[i&1].update(1, M, 0);
    // sum[i&1].update(1, M, 0);
    // a[i]/j == k
    for (j = 1; j*j <= a[i]; ++j) {
      k = l = a[i]/j;
      r = (a[i]+j-1)/j;
      Mint _cnt = cnt[~i&1].query(r, M);
      cnt[i&1].update(l, _cnt);
      sum[i&1].update(l, sum[~i&1].query(r, M)+(j-1)*_cnt);
      cnt[~i&1].update(r, M, 0);
      sum[~i&1].update(r, M, 0);
    }
    // a[i]/k == j
    for (j = k-1; j; --j) {
      l = max(1, a[i]/(j+1));
      // r = a[i]/j;
      Mint _cnt = cnt[~i&1].query(j, M);
      cnt[i&1].update(j, _cnt);
      sum[i&1].update(j, sum[~i&1].query(j, M)+(l-1)*_cnt);
      cnt[~i&1].update(j, M, 0);
      sum[~i&1].update(j, M, 0);
    }
    cnt[i&1].update(a[i], cnt[i&1].query(a[i])+1);
    #ifdef DEBUG
    for (int j = 1; j <= M; ++j) cout << cnt[i&1].query(j) << " \n"[j==M];
    for (int j = 1; j <= M; ++j) cout << sum[i&1].query(j) << " \n"[j==M];
    #endif
    ans += sum[i&1].query(1, M);
  }
  cout << ans << '\n';
}
