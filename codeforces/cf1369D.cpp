/*
 * @Author: Kaizyn
 * @Date: 2020-06-23 22:01:52
 * @LastEditTime: 2020-06-23 22:51:10
 */ 
#include <bits/stdc++.h>
...
mint max(const mint &a, const mint &b) {
  return (a-b).v <= 10 ? a : b;
}

inline void solve() {
  cin >> n;
  cout << max(dp[n][0], dp[n][1])*4 << endl;
}
...
signed main() {
  ...
  dp[4][1] = dp[4][0] = 1;
  for (int i = 5; i < N; ++i) {
    dp[i][1] = dp[i-2][0]*2+dp[i-1][0]+1;
    dp[i][0] = max(dp[i-2][0], dp[i-2][1])*2+max(dp[i-1][1], dp[i-1][0]);
