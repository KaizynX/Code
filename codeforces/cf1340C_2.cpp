 ...
 * @LastEditTime: 2020-04-24 01:00:58
 */
#include <bits/stdc++.h>
...
inline void solve()
{
    ...
    while (q.size()) {
        ...
        #endif
        if (x-1 > 0) {
            int rest = (y ? y : g)-(d[x]-d[x-1]);
            if (rest >= 0) {
                if (dp[x-1][rest] > dp[x][y]+(rest == 0)) {
                    dp[x-1][rest] = dp[x][y]+(rest == 0);
                    if (!vis[x-1][rest]) q.push({x-1, rest}), vis[x-1][rest] = 1;
                }
            }
        }
        if (x+1 <= m) {
            int rest = (y ? y : g)-(d[x+1]-d[x]);
            if (rest >= 0) {
                if (dp[x+1][rest] > dp[x][y]+(rest == 0)) {
                    dp[x+1][rest] = dp[x][y]+(rest == 0);
                    if (!vis[x+1][rest]) q.push({x+1, rest}), vis[x+1][rest] = 1;
