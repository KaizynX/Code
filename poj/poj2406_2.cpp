 ...
 * @LastEditTime: 2020-05-04 20:26:30
 */
#include <cmath>
#include <cstdio>
#include <cstdlib>
...
int main() {
    ...
    while (scanf("%s", s) && strcmp(s, ".")) {
        ...
        rmq[rk[1]] = n;
        for (int i = rk[1]-1; i; --i) {
            rmq[i] = min(rmq[i+1], ht[i+1]);
        }
        for (int i = rk[1]+1; i <= n; ++i) {
            rmq[i] = min(rmq[i-1], ht[i]);
        }
        int res = 1;
        for (int i = 1; i*i <= n; ++i) {
            if (n%i) continue;
            if (rmq[rk[i+1]] >= n-i) res = max(res, n/i);
            if (i == n/i) continue;
            if (rmq[rk[n/i+1]] >= n-n/i) res = max(res, i);
        }
        printf("%d\n", res);
    }
}