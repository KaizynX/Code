/*
 * @Author: Kaizyn
 * @Date: 2020-03-16 14:32:15
 * @LastEditors: Kaizyn
 * @LastEditTime: 2020-03-16 14:47:04
 * @FilePath: \Code\poj\poj3101_2.cpp
 */
#include <map>
#include <cmath>
#include <cstdio>
...
inline void print(const int arr[])
{
    big = 1;
    for (int i = 1; i <= E.tot; ++i) if (arr[i])
        big *= qpow(E.prime[i], arr[i]);
    cout << big;
}

inline void ope(int arr[], int num, const int &tag)
{
    for (int i = 1, k; i <= E.tot && E.prime[i] <= num; ++i) {
        if (num%E.prime[i]) continue;
        k = 0;
        while (num%E.prime[i] == 0) {
            num /= E.prime[i];
            ++k;
        }
        if (tag) arr[i] = max(arr[i], k);
        else arr[i] = min(arr[i], k);
    }
}
